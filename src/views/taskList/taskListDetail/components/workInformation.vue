<template>
  <el-card>
    ...
    <div>
      <div class="flex add pointer fs_16" @click="add">添加新纪录</div>
      <div class="text fs_12" v-for="(item, index) in common.list" :key="index">
        <div class="top flex fs_12 textCenter">
          <ul class="first">
          ...
          <ul>
            ...
            <li>{{ dayjs(item.createTime).format("YYYY-MM-DD HH:mm:ss") }}</li>
          </ul>
          <ul>
      ...
      <el-dialog v-model="common.dialogVisible" title="添加新纪录" width="600px" @close="close">
...
const taskId = useTaskListDetailStore().taskId
interface obj {
  createTime: string,
  creatorName: string,
  jumpUrl: string
...
const openUrlAxios = async (kindId: string) => {
  let res = await openUrl({ taskId, kindId })
  common.url = res.data
}
const close = () => {
  common.url = ''
